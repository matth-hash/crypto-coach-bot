    ...
    for t in trades:
        try:
            ...
            await save_journal_trade(
                user_id=user_id, symbol=symbol, side=t["side"],
                entry_price=float(t["price"]), exit_price=None,
                amount=float(t["cost"]) if t.get("cost") else float(t["amount"]) * float(t["price"]),
                emotion="neutral", reason=f"Import {exchange_id}",
            )
            imported += 1
        except Exception:
            continue
...
async def process_entry_price(message: Message, state: FSMContext):
    ...
    try:
        ...
        errors = {"fr": "❌ Prix invalide. Ex: *65000* ou *2500.50*", "en": "❌ Invalid price. E.g.: *65000* or *2500.50*", "es": "❌ Precio inválido. Ej: *65000* o *2500.50*", "pt": "❌ Preço inválido. Ex: *65000* ou *2500.50*"}
    ...
    texts = {"fr": "💰 *Prix de sortie* en $ _(ou ignore si position ouverte)_ :", "en": "💰 *Exit price* in $ _(or skip if position still open)_ :", "es": "💰 *Precio de salida* en $ _(o ignora si posición abierta)_ :", "pt": "💰 *Preço de saída* em $ _(ou ignore se posição ainda aberta)_ :"}
...
async def process_exit_price(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer({"fr": "❌ Prix invalide. Ex: *68000*", "en": "❌ Invalid price. E.g.: *68000*", "es": "❌ Precio inválido. Ej: *68000*", "pt": "❌ Preço inválido. Ex: *68000*"}.get(lang, "❌"), parse_mode="Markdown")
        return
...
async def process_amount(message: Message, state: FSMContext):
    ...
    builder.adjust(2)
    texts = {"fr": "🧠 *État émotionnel* au moment du trade :", "en": "🧠 *Emotional state* at the time of the trade:", "es": "🧠 *Estado emocional* en el momento del trade:", "pt": "🧠 *Estado emocional* no momento do trade:"}
    await message.answer(texts.get(lang, texts["fr"]), reply_markup=builder.as_markup(), parse_mode="Markdown")
...
async def cb_journal_emotion(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_reply_markup()
    texts = {"fr": "✍️ *Raison du trade* — pourquoi tu as pris cette position ? _(1-2 phrases)_", "en": "✍️ *Trade reason* — why did you take this position? _(1-2 sentences)_", "es": "✍️ *Razón del trade* — ¿por qué tomaste esta posición? _(1-2 frases)_", "pt": "✍️ *Motivo do trade* — por que você tomou essa posição? _(1-2 frases)_"}
...
async def cb_journal_postmortem(callback: CallbackQuery, state: FSMContext):
    ...
    # Gate freemium — 1 post-mortem/jour
    limit_check = await check_free_limit(user_id, "postmortem")
    if not limit_check["allowed"]:
        upgrade_msgs = {
            "fr": (
                f"⛔ *Limite atteinte* ({limit_check['used']}/{limit_check['limit']} post-mortem aujourd'hui)\n\n"
                f"Le plan gratuit est limité à *1 post-mortem IA par jour*.\n"
                f"Passe Premium pour des analyses illimitées ! 🚀\n\n"
                f"/premium — Voir les offres"
            ),
            "en": (
                f"⛔ *Limit reached* ({limit_check['used']}/{limit_check['limit']} post-mortem today)\n\n"
                f"The free plan is limited to *1 AI post-mortem per day*.\n"
                f"Upgrade to Premium for unlimited analysis! 🚀\n\n"
                f"/premium — View plans"
            ),
            "es": (
                f"⛔ *Límite alcanzado* ({limit_check['used']}/{limit_check['limit']} post-mortem hoy)\n\n"
                f"El plan gratuito está limitado a *1 post-mortem IA por día*.\n"
                f"¡Actualiza a Premium para análisis ilimitados! 🚀\n\n"
                f"/premium — Ver planes"
            ),
            "pt": (
                f"⛔ *Limite atingido* ({limit_check['used']}/{limit_check['limit']} post-mortem hoje)\n\n"
                f"O plano gratuito é limitado a *1 post-mortem IA por dia*.\n"
                f"Atualize para Premium para análises ilimitadas! 🚀\n\n"
                f"/premium — Ver planos"
            ),
        }
        await callback.message.edit_text(upgrade_msgs.get(lang, upgrade_msgs["en"]), parse_mode="Markdown")
        await callback.answer()
        return

    trades = await get_journal_trades(user_id, limit=10)
    if not trades:
        empty = {"fr": "📭 Pas encore de trades dans ton journal.\n\nEnregistre au moins un trade pour obtenir une analyse.", "en": "📭 No trades in your journal yet.\n\nRecord at least one trade to get analysis.", "es": "📭 Aún no hay trades en tu diario.", "pt": "📭 Ainda não há trades no seu diário."}
        await callback.message.edit_text(empty.get(lang, empty["fr"]))
        await callback.answer()
        return
    ...
    # Incrément compteur
    await increment_daily_usage(user_id, "postmortem")
